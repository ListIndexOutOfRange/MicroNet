...
class BasicBlockSmall(nn.Module):
    ...
    def __init__(self, in_planes, planes, activation, attention, sym, stride = 1, option = 'A'):
...
class ResNetSmall(nn.Module):
    ...
    def _make_layer(self, block, planes, num_blocks, stride, activation, attention, sym):
        strides = [stride] + [1] * (num_blocks - 1)
        layers = []
        for stride in strides:
            layers.append(block(self.in_planes, planes, activation, attention, sym, stride))
...
def ResNet(arch, num_classes = 100, activation='relu', attention=False, sym=False):
