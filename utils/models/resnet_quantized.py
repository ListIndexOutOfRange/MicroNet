...
class BasicBlockSmall(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride = 1, option = 'A'):
        ...
        self.conv1 = qnn.QuantConv2d(in_planes, planes, kernel_size = 3, stride = stride, padding = 1, bias = False,
                                     weight_quant_type=QuantType.INT, weight_bit_width=8)
        self.bn1 = nn.BatchNorm2d(planes)
        #self.conv2 = nn.Conv2d(planes, planes, kernel_size = 3, stride = 1, padding = 1, bias = False)
        self.conv2 = qnn.QuantConv2d(planes, planes, kernel_size = 3, stride = 1, padding = 1, bias = False,
                                     weight_quant_type=QuantType.INT, weight_bit_width=8)
...
class ResNetSmall(nn.Module):
    ...
    def __init__(self, block, num_blocks, num_classes = 10):
        ...
        self.conv2 = qnn.QuantConv2d(3, 16, kernel_size = 3, stride = 1, padding = 1, bias = False,
                                     weight_quant_type=QuantType.INT, weight_bit_width=8)
        self.bn1 = nn.BatchNorm2d(16)
        self.layer1 = self._make_layer(block, 16, num_blocks[0], stride = 1)
        self.layer2 = self._make_layer(block, 32, num_blocks[1], stride = 2)
        self.layer3 = self._make_layer(block, 64, num_blocks[2], stride = 2)
        self.linear = qnn.QuantLinear(64, num_classes, bias=False, 
